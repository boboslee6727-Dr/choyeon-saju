...
★ [도트(•) 양식 라인 맞춤 필수]: 웹 화면 깨짐과 외계어 기호 노출을 방지하기 위해, 아스테리스크(*, **) 기호를 절대 사용하지 마십시오. 반드시 줄 맨 첫 칸에 특수문자 도트(•)를 사용하고 뒤이어 한 칸 띄운 후 아래 예시의 규격 틀에 맞게 실제 해당 사주의 숫자와 간지만 대입하여 정렬 출력하십시오. (줄 맨 앞 공백/탭 절대 금지)
(출력 양식 예시)
