...
import streamlit.componeimport streamlit as st
import pandas as pd
import json
import os
import math
...
"""
                ...
                st.error(f"AI 연산 오류: {e}")nts.v1 as components

# ==============================================================================
# 0. VIP 인셋 프레임 및 초강력 프린트 CSS 
# ==============================================================================
st.set_page_config(- [천간 합/충 짝짓기 오류(환각) 절대 금지] 천간의 합(合)은 '甲己, 乙庚, 丙辛, 丁壬, 戊癸' 이고, 천간의 충(沖)은 '甲庚, 乙辛, 丙壬, 丁癸' 뿐입니다. 절대 '갑경합', '을기충' 등 글자 짝을 잘못 지어 명리학에 없는 거짓 용어를 지어내지 마십시오. 충(갈등/변화)의 상황에 합(合)이라는 단어를 쓰는 실수를 엄금합니다.page_title="초연 전통명리 사주풀이 Ver 13.32", layout="wide")
...
[🌟 대중 친화적 하이브리드 통변 강제 지시]
- [천간충 환각 절대 금지] 천간의 충(沖)은 오직 '甲庚, 乙辛, 丙壬, 丁癸' 뿐입니다. 절대 '을기충' 등 명리학에 없는 거짓 충을 지어내지 마십시오.
- 모든 명리 용어(십성, 신살, 묘유충 등)는 절대로 단독으로 쓰지 마십시오.
- 반드시 [대중이 이해하기 쉬운 현대적 구어체 표현] + (명리용어) 형태로 병기하십시오.
  예시) "가을의 서늘함과 봄의 생동감이 부딪히는 현상(卯酉충)"
- [한자 100% 표기 규칙] '甲목', '己토' ❌ -> 반드시 '甲木', '己土', '亥水', '甲庚충' 등 100% 한자(漢字)로 표기하십시오.
